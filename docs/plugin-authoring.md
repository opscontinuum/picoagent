...
a slightly longer list because a model-composed command also runs the interpreter this harness
lives on (`VIRTUAL_ENV`, `PYTHONPATH`, `PYTHONHOME`), and widened by the user's
`shell_env_allow` rather than by a `pass_env` argument. If your plugin replaces `shell`, start
from one of those two rather than from `os.environ`.
