...
*(2026-09-09 editor's note: this section was recorded on branch
`docs/document-ingestion-plugins` before the 2026-09-08 plugin extraction; plugins now land
in `opscontinuum/picoagent-plugins`, not `examples/plugins/`. `doc-ooxml` and `doc-pdf` are
built and live there; `instrument-cache` is not yet built. One divergence from this
section as recorded: the built `doc-pdf` declares `python_deps = ["pymupdf>=1.24"]` -
the AGPL-3.0/commercial library this section flags - rather than the pdfplumber
substitute it recommends, so the licence question below is answered by declining the
plugin, or still open to a backend swap.)*
...
| [opscontinuum/picoagent-plugins](https://github.com/opscontinuum/picoagent-plugins) `doc-ooxml`, `doc-pdf` | Reads `.docx`/`.xlsx` and PDFs as evidence for the audit and authoring paths (section 6) |
| `opscontinuum/oci-itscp` | The worked ITSCP reference: Oracle EBS on Exadata, Ashburn to Phoenix |

Not built: the `instrument-cache` plugin of section 6, the record-of-changes generator of
